...
async def _handle_realtime_sideband_function_call(
    ...
    *,
    ...
    _log.info(
        "Realtime voice sideband response.create posted name=%s call_id=%s session=%s",
        name,
        call_id,
        session.call_id,
    )
...
async def desktop_upload_image(request: Request):
    ...
    Hermes Desktop may run on a Mac while its backend runs on a Pi. In that
    mode pasted screenshots initially exist only under the Mac's Application
    Support directory, so the backend cannot attach them by path. This endpoint
